#include "course_1_7_1_camera_circle.h"
#include <QDir>
#include <QDebug>
#include "stb_image.h"
#include <qmath.h>

Course_1_7_1_camera_circle::Course_1_7_1_camera_circle()
    :Course()
{
    initializeOpenGLFunctions();

    glEnable(GL_DEPTH_TEST);
    QString shaderPath = QDir::currentPath() + "/shaders/1.getting_started/7.1.camera_circle/";
    mProgram = new QOpenGLShaderProgram();
    mProgram->addShaderFromSourceFile(QOpenGLShader::Vertex,shaderPath+"7.1.camera.vs");
    mProgram->addShaderFromSourceFile(QOpenGLShader::Fragment,shaderPath+"7.1.camera.fs");
    mProgram->link();
...
void Course_1_7_1_camera_circle::render()
{
    Course::render();
    ...
    mView.setToIdentity();
    float radius = 10.0f;
    float camX   = sin(mDuration) * radius;
    float camZ   = cos(mDuration) * radius;
    mView.lookAt(QVector3D(camX, 0.0f, camZ), QVector3D(0.0f, 0.0f, 0.0f), QVector3D(0.0f, 1.0f, 0.0f));
...
QList<QtProperty *> Course_1_7_1_camera_circle::getPropertyList(QtVariantPropertyManager *manager)
{
    GETPROPERTYLIST(Course_1_7_1_camera_circle)
}
