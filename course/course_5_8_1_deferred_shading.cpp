#include "course_5_8_1_deferred_shading.h"
#include <QDir>
#include <QDebug>
#include "stb_image.h"
#include <QResizeEvent>

Course_5_8_1_deferred_shading::Course_5_8_1_deferred_shading()
    :CourseWithCamera()
    ,cubeVAO(0)
    ,cubeVBO(0)
    ,quadVAO(0)
...
void Course_5_8_1_deferred_shading::init(GlWindow *glWindow)
{
    setGlWindow(glWindow);
    mCamera = Camera(QVector3D(0.0f, 0.0f, 5.0f));
    glEnable(GL_DEPTH_TEST);

    QString shaderPath = QDir::currentPath() + "/shaders/5.advanced_lighting/8.1.deferred_shading/";
    shaderGeometryPass.addShaderFromSourceFile(QOpenGLShader::Vertex,shaderPath+"8.1.g_buffer.vs");
    shaderGeometryPass.addShaderFromSourceFile(QOpenGLShader::Fragment,shaderPath+"8.1.g_buffer.fs");
    shaderGeometryPass.link();

    shaderLightingPass.addShaderFromSourceFile(QOpenGLShader::Vertex,shaderPath+"8.1.deferred_shading.vs");
    shaderLightingPass.addShaderFromSourceFile(QOpenGLShader::Fragment,shaderPath+"8.1.deferred_shading.fs");
    shaderLightingPass.link();

    shaderLightBox.addShaderFromSourceFile(QOpenGLShader::Vertex,shaderPath+"8.1.deferred_light_box.vs");
    shaderLightBox.addShaderFromSourceFile(QOpenGLShader::Fragment,shaderPath+"8.1.deferred_light_box.fs");
    shaderLightBox.link();
...
void Course_5_8_1_deferred_shading::render()
{
    Course::render();
...
QList<QtProperty *> Course_5_8_1_deferred_shading::getPropertyList(QtVariantPropertyManager *manager)
{
    GETPROPERTYLIST(Course_5_8_1_deferred_shading)
}

void Course_5_8_1_deferred_shading::Resize(QResizeEvent *event)
{
    SCR_WIDTH = 0;
    SCR_HEIGHT = 0;
...
void Course_5_8_1_deferred_shading::renderCube()
{
...
void Course_5_8_1_deferred_shading::renderQuad()
{
    if (quadVAO == 0)
    {
        float quadVertices[] = {
